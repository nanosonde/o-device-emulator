...
def test_olt_discovery_body_uses_controller_id_convention():
    ...
    `controllerSetting`/`controllerId`). The OLT discovery controller setting
    maps its `controllerId` field with JSON key `"id"` and the body
    wrapper maps `controllerSetting` with JSON key
    `"controller"`. `deviceMisc` is the base device misc shape
    (`modelType`/`category`/`supportCluster`); `upTime` is a JSON integer.
    CONFIRMED against a live controller v6.2 (see doc/DEVICE_PROTOCOL.md §4.4)."""
    body = build_olt_discovery_body(
        ip="192.168.56.80",
        model="DS-P7001-08",
        model_version="1.0",
        firmware_version="1.0.0 Build 20230101 Rel.12345",
    ...
    assert body["deviceInfo"]["modelVersion"] == "1.0"
    assert body["deviceInfo"]["firmwareVersion"].startswith("1.0.0")
    assert body["deviceInfo"]["hardwareVersion"] == "1.0"
    # upTime is a JSON integer (Long), not a string.
    assert isinstance(body["deviceInfo"]["upTime"], int)
    # deviceMisc is the base device misc shape (modelType/category/supportCluster).
