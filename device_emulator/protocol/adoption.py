...
def build_negotiation_body(
    ...
    *,
    ...
    dev_cap: dict[str, Any] | None = None,
    device_misc: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    `dev_cap` overrides the empty ``devCap`` placeholder — pass
    ``{"supportTerminal": True}`` for devices that should appear in the
    Terminal device picker. ``device_misc`` is merged into ``deviceMisc``
    (on top of the default ``customizeRegion``).
    """
    misc: dict[str, Any] = {"customizeRegion": country_code}
    if device_misc:
        misc.update(device_misc)
    return {
        "key": "",
