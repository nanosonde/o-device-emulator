...
# 2.3 (ECSP first-version enum, V2), so advertise "2.3.0" (major 2 / minor 3) to be
...
# for "ap", "switch", "gateway", "olt"; consistent with the controller's
# ECSP device-type enum and the common device-type enum — OLT live-validated
# against controller v6.2.14.11; see doc/DEVICE_PROTOCOL.md §4.4).
DEVICE_TYPE_AP = "ap"
...
# File-transfer message types, exchanged on the management channel (29814).
# The controller REQUESTS a file (FILE_TRANSFER_REQUEST_V2) and the device
# RESPONDS with one or more FILE_TRANSFER_RESPONSE_V2 messages, each carrying
# one base64-encoded partition. CONFIRMED against the live controller: only
# FILE_TRANSFER_RESPONSE_V2 is accepted from a device (routed to the
# file-transfer service); FILE_TRANSFER_REQUEST_V2 hits the default branch and
# the controller closes the channel.
MESSAGE_TYPE_FILE_TRANSFER_REQUEST_V2 = 0x160000
MESSAGE_TYPE_FILE_TRANSFER_RESPONSE_V2 = 0x170000
