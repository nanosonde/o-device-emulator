...
def switch_lldp_section(links: list[LinkNeighbor]) -> dict[str, Any]:
    """The switch ``lldp`` INFORM section. Uses the ``port`` / ``standardPort``
    keys the controller expects for LLDP (the topology map is happy with
    either, but the Tools -> LLDP Neighbor Table view keys its rows on
    ``standardPort`` and errors if it is missing)."""
    return {
        "lldp": {
            "lldps": [
                {
...
def switch_port_section(links: list[LinkNeighbor]) -> dict[str, Any]:
    ...
    ``standardPort`` uses the controller's ``unit/slot/port`` form (``1/0/N``);
    a bare ``N`` fails the controller's port lookup."""
    return {
        "port": {
            "ports": [
                {
