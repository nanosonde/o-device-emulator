...
class WiredDevice(Device):
    ...
    def manage_dev_cap(self) -> dict[str, Any]:
        ...
        # Terminal device picker includes this switch/gateway. The
        # controller checks `oswImage.bk()` (mapped from the
        # `terminalSetting` component in components_v2) and the frontend
        # additionally checks `devCap.terminalSupport`. See
        # doc/DEVICE_PROTOCOL.md §10.6.
    ...
    def build_manage_negotiation_body(self, controller_id: str) -> dict[str, Any]:
        ...
        # Merge the profile's devCap with the device's terminal support flag.
        dev_cap = dict(self.profile.DEV_CAP)
        dev_cap.update(self.manage_dev_cap())
        return {
            "key": "",
            "configVersion": "0",
